...
use crate::{basic::*, Mainstruct, Message};
use iced::{Button, Color, Column, Container, Element, Length, Row};

pub fn reviewscreen(selfx: &mut Mainstruct) -> Element<Message> {
    let timer = h4(selfx.settings_usize[5], get_time(selfx));
    ...
    let text1 = h2(selfx.settings_usize[3], selfx.letters.concat())
        .height(Length::Units(150))
        .color(colours[selfx.colour as usize]);
    let buttonnames = ["gotomain", "shift", "submit", "space", "delete", "next"];
    let [state0, state1, state2, state3, state4, state5] =
    ...
    for i in selfx.lang_one_states.values_mut() {
        buttonone.push(add_button(
            i,
            shiftfn(selfx.shift, selfx.langone[x].to_string()),
            Message::LetterPressed(selfx.langone[x].to_string()),
    ...
    for i in selfx.lang_two_states.values_mut() {
        buttontwo.push(add_button(
            i,
            shiftfn(selfx.shift, selfx.langtwo[x].to_string()),
            Message::LetterPressed(selfx.langtwo[x].to_string()),
            selfx.settings_usize[6],
        ));
        x += 1;
    }
    ...
    for i in selfx.punctuation_states.values_mut() {
        buttons.push(add_button(
            i,
            selfx.punctuation[x].to_string(),
            Message::LetterPressed(selfx.punctuation[x].to_string()),
            selfx.settings_usize[6],
        ));
        x += 1;
    }
    ...
    let exit = add_button(
        state0,
        String::from("Exit Testing"),
        Message::ButtonPressed("Exit".to_string()),
        selfx.settings_usize[6],
    );
    let shift = add_button(
        state1,
        String::from("Shift"),
        Message::ButtonPressed("Shift".to_string()),
        selfx.settings_usize[6],
    );
    let submit = add_button(
        state2,
        String::from("submit"),
        Message::ButtonPressed("Submit".to_string()),
        selfx.settings_usize[6],
    );
    let space = add_button(
        state3,
        String::from("space"),
        Message::ButtonPressed("Space".to_string()),
        selfx.settings_usize[6],
    );
    let delete = add_button(
        state4,
        String::from("delete"),
        Message::ButtonPressed("Delete".to_string()),
        selfx.settings_usize[6],
    );
    let next = add_button(
        state5,
        String::from("next"),
        Message::ButtonPressed("Next".to_string()),
        selfx.settings_usize[6],
    );
    ...
    let column1 = Column::new()
        .push(utilrow.width(Length::Fill))
        .push(english)
        .push(text1)
        .push(userrow)
        .push(row1)
