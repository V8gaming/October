...
use crate::{
    load::{loadalphabet, loadhashmaps, loadlangsettings},
    Mainstruct,
};
...
pub fn nextfn(selfx: &mut Mainstruct) {
    ...
    selfx.word_index = thread_rng().gen_range(0..selfx.langonevec.len());
    selfx.letters.clear();
    selfx.colour = 0;
}
...
pub fn sumbitfn(selfx: &mut Mainstruct) {
    ...
    if selfx.letters.concat() == selfx.langtwovec[selfx.word_index] {
        //println!("true");
        selfx.colour = 2;
        selfx.screen = 2;
    } else {
